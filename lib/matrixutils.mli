...
(** Raised when there is an unknown/missing matrix operation *)
...
(** A variant representing a job that has been split into per-worker chunks for
...
(** Reads a floating-point matrix from the input channel using the same format
    produced by [print_matrix]. *)
...
(** Reads a CSV file into a floating-point matrix. Raises [MatrixNotReadable] on
    malformed content. *)
...
(** Parses a matrix operation (add, subtract, multiply, scale, transpose) from
    [rest_string], consuming it and returning its string name. *)
